...
if(isset($_POST["rendben"]))
{
    ...
    $mime = array("image/gif", "image/png", "image/jpg", "image/jpeg");
	 ...
	 function Inseert($alias, $cim, $tartalom, $fohir, $leiras, $statusz, $foto, $hiriroid)
	 {
		...
		$sql = "INSERT INTO `hirek`(`alias`, `cím`, `tartalom`, `létrehozás`, `főhír`, `leírás`, `státusz`, `megjelenőkép`, `hiriroid`) VALUES ('{$aliasp}','{$cimp}','{$tartalomp}',Now(),'{$fohirp}','{$leirasp}','{$statuszp}','{$fotop}', '{$hiriroidp}')";
	 ...
	 else
	 {
		...
		if ($fohir == 1)
		{
			...
			$mennyifohir = "select id from hirek where főhír = 1";
			$mennyifohireredmeny = mysqli_query($dbconn, $mennyifohir);

			if(mysqli_num_rows($mennyifohireredmeny) >= 6)
			{
				
			$idle = "select id from hirek where főhír = 1 order by létrehozás asc limit 1";
	...
	//érvényes tartalom 
					if (mysqli_num_rows($idleeredmeny) == 1) {
						...
						$update = "Update hirek set főhír = 0 where id = {$regiid}";
