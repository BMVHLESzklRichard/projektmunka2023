...
$sql = "SELECT hirek.id,hirek.tartalom,hirek.letrehozas,hirek.modositas,hirek.leiras, hirek.kulcsszavak,hirek.cim,projectacc.name, projectacc.foto
        FROM `hirek` 
        INNER JOIN projectacc ON hirek.hiriroid = projectacc.id
        WHERE alias = '{$alias}'
        LIMIT 1";
...
if(mysqli_num_rows($eredmeny) == 1)
{
            ...
            $sql2 = "SELECT hozzaszolasok.id,hozzaszolasok.hirszoveg,projectacc.name,projectacc.foto,hozzaszolasok.hirdatum FROM `hozzaszolasok` inner join hirek on hirek.id = hozzaszolasok.hirid inner join projectacc on projectacc.id = hozzaszolasok.accid WHERE hirek.alias = '{$alias}'";
        $eredmeny2 = mysqli_query($dbconn,$sql2);
            ...
            if (mysqli_num_rows($eredmeny2) >= 1)
            {
                ...
                if (isset($_POST['new'])) {
                    $szoveg = strip_tags(trim(strtoupper($_POST['szoveg'])));
                    $sql = "INSERT INTO `hozzaszolasok`(`hirid`, `hirszoveg`, `hirdatum`, `accid`) VALUES ('{$id}','{$szoveg}',NOW(),'{$bejelentid}')";
                ...
                $hozzaszolas.= "</div>
                <h1>Ha kommentelni akarsz jelentkezz be vagy regisztrálj!</h1></div>";
            }
