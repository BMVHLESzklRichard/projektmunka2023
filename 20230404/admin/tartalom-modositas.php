	...
	//változók ellenőrzés
	 ...
	 else
	 {
        ...
        $id= (int)$_GET['id'];
		 ...
		 $sql = "UPDATE `news` SET `alias`='{$alias}',`title`='{$cim}',`content`='{$tartalom}',`modDate`=NOW(),`description`='{$leiras}',`status`='{$statusz}', $fotoQuery `mainNews`='{$fohir}' WHERE `id` = {$id}";
